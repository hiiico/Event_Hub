...
//    @Query("{ 'attendees': { $elemMatch: { $id: ?0 } } }")
//    Page<Event> findByAttendeeId(@Param("userId") String userId, Pageable pageable);
//    @Query("{ 'attendees': { $elemMatch: { $ref: 'users', $id: ?0 } } }")
//    Page<Event> findByAttendeeId(String userId, Pageable pageable);
@Query("{ 'attendees.$id': ?0 }")
Page<Event> findByAttendeeId(ObjectId userId, Pageable pageable);
}
